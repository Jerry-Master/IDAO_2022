...
class MyMEGNetModel(GraphModel):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            pre_layer: (Function) model between input and first MEGNetLayer
            post_layer: (Function) model between concatenate and output
...
def make_megnet_model(
    ...
    **kwargs,
    ...
    Args:
        pre_layer: (Function) model between input and first MEGNetLayer
        post_layer: (Function) model between concatenate and output, if None 
            the model returns the feature vector after concatenate
        nfeat_edge: (int) number of bond features
        nfeat_global: (int) number of state features
        nfeat_node: (int) number of atom features
        nblocks: (int) number of MEGNetLayer blocks
    ...
    # a block corresponds to preprocessing layer + one MEGNetLayer layer
    # Note the first block does not contain the preprocessing layer since
    ...
    # Add post processing layer or output the feature vector
    if post_layer is not None:
        final_vec = post_layer(name_prefix='post_process_layer')(final_vec)
    else:
        return Model(inputs=[x1, x2, x3, x4, x5, x6, x7], outputs=final_vec)
